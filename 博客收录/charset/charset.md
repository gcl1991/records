...
UTF-16将这个范围内的代码点编码为16位代码单元，它们在数值上等于相应的码点。

+ U+10000 to U+10FFFF

通过以下方案，将补充平面码点编码为代理对（两个16-bit代码单元）：

（1）从代码点中减去0x010000，U+10000\~U+10FFFF变为0x00000~0xFFFFF（20bit）。

（2）前十位（0x0000\~0x03FF范围内的数字）被添加到0xD800，以给出第一个16位代码单元或高代理，其范围为0xD800\~0xDBFF。

（3）低十位（也位于0x0000\~0x03FF范围内）被添加到0xDC00，以提供第二个16位代码单元或低代理，其范围为0xDC00\~0xDFFF。

![](utf-16.jpg)

U+D800 to U+DFFF差值为2048，0xD800-0xDBFF共1024，0xDC00-0xDFFF共1024，1024*1024共0xFFFFF

例子

考虑U + 10437的utf-16编码D801 DC37

（1）从0x10437减去0x10000。结果为0x00437，0000 0000 0100 0011 0111。

（2）将其分为高10位值和低10位值：0000000001和0000110111。

（3）0xD800 + 0x0001 = 0xD801，0xDC00 + 0x0037 = 0xDC37。

## UTF-8

由于ASCII字符经过UTF-16编码后高字节始终是0x00，一方面很多C语言的函数都将此字节视为字符串末尾从而导致无法正确解析文本，另一方面存储和处理效率相对低下，后来聪明的人们发明了UTF-8编码，解决了这个问题。

UTF-8编码方案采用1-4个字节来编码字符。

0x00-0x7F和ASCII编码一摸一样。

0x7F-0x10FFFF，使用2-4个字节来表示，其中，首字节前置1的数目代表正确解析所需要的字节数，剩余字节的高2位始终是10。
